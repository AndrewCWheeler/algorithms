...
class SLNode {
  // The constructor is built to take 1 parameter, being the value of the node we want to create
  constructor(val) {
...
class SLList {
  ...
  //EXAMPLE: myList = 7 -> 5 -> 3 -> 10 -> 1 -> calling myList.secondToLast() would return the 10 -> node
  secondToLast() {
    ...
    // One element check:
      ...
      // We want to move both down the line until the runner is at the last node. If the runner is at the last node, the walker will be at the second to last node!
  ...
  // Write a method that takes a singly linked list as a parameter, and concatenates it to the current list.

  // EXAMPLE: If myList = 7 -> 5 -> 3 -> and otherList is 10 -> 1 -> 8 -> and you call myList.concat(otherList) the outcome would be that myList is now 7 -> 5 -> 3 -> 10 -> 1 -> 8 ->
  concat(list) {
    ...
    // Once the runner is at the end of the list, set its .next to the head of the second list.
  ...
  // EXAMPLE: if myList = 10 -> 7 -> 5 -> 3 -> 1 -> and you call myList.splitOnVal(5) myList will be 10 -> 7 -> and the method would return a new SLL of 5 -> 3 -> 1 ->
  splitOnVal(value) {
    ...
    // Now we need to check if the head's value is the one we're trying to split from
      ...
      // If it is, then basically we clear the current list and return a new list that contains everything that was in the current list.
      ...
      while (runner.next != null) {
        // Let's check to see if the next node's value is the one we're trying to split at. If it is, we'll create our new list and call it a day!
  ...
  // Write a method that takes a value, and will remove the first instance of a node with that value in the singly linked list.

  // EXAMPLE: with a list of 10 -> 7 -> 3 -> 7 -> 6 ->
  // If you call myList.removeNode(7) the list will become 10 -> 3 -> 7 -> 6 ->

  // NOTE: Removing a node is as simple as redirecting the previous node's .next to the removed node's .next
  removeNode(value) {
    if (this.isEmpty()) {
  ...
  // Write a method that will return a boolean based on whether or not the Singly Linked List contains a node with a given value.

  // EXAMPLE: If the singly linked list is 7 -> 5 -> 9 -> 2 -> and I call myList.contains(9) it should return true. If on the same list I call myList.contains(11) it should return false.
  contains(value) {
    ...
    // If we reached the end of the list and found no matches, the list must not contain the value, so return false!
    return false;
  }

  // Write a method that will remove the last node in a SLL and return it.

  // EXAMPLE: If the singly linked list is 11 -> 2 -> 7 -> 6 -> and I call myList.removeFromBack() the list should now be 11 -> 2 -> 7 ->
  removeFromback() {
    ...
    // Otherwise, let's check to see if the list has only 1 element!
      ...
      while (runner.next != null) {
        // Setting walker to runner before moving runner to runner's next makes it so once runner is the LAST node, walker will be the SECOND TO LAST node
        walker = runner;
        runner = runner.next;
      }
  ...
  // Write a method that will create a new node, add it to the front of the singly linked list, and reassign the head to the new node.
  addToFront(value) {
    // This one is pretty simple! The case for an empty list vs a non-empty list is exactly the same!

    // Create the new node
    let newNode = new SLNode(value);
    // Then set the new node's .next to the current head. IF THE LIST IS EMPTY! newNode.next will be null. Which is what it should be anyway if there's nothing after it.
    // IF THE LIST IS NOT EMPTY! newNode.next will be the current head of the list, which is exactly what we want!
  ...
  // Write a method that will remove the head node from a singly linked list, and then reassign the head to the next node. Return the node that was removed
  removeFromFront() {
    // We should first check to see if the list is empty, because if it is, there's nothing to remove!
    if (this.isEmpty()) {
    ...
    // Similar to the previous algorithm (addToFront), if the original head's .next is null, and you remove from the front, this.head is moved to null, which is fine! Because removing from the front of an SLL with only one node is the same as emptying it!!
  }

  // Write a method that will return a boolean depending on whether or not the singly linked list is empty or not.
  isEmpty() {
    // An empty list in its most simplified form is a list with a head that is null.
  ...
  // Write a method that is given a value, and adds a new node to the end of a SLL where that new node has that value.
  addToBack(value) {
    // First we need to check if the list is empty
    if (this.isEmpty()) {
      // If it is, just set the head to a new node, because adding to the back of an empty list is the same as just setting the head to a node
    ...
    // OTHERWISE
      ...
      // Once the runner is at the end of the list, we set its .next to be a new node
...
// This is the class for a Stack where everything is built using methods from the SLL class we've been working with
class Stack {
  constructor() {
    this.data = new SLList();
  }

  // Write a method to push a value into our stack using the methods we've built in our singly linked list class
  push(value) {
    this.data.addToFront(value);
    return this;
  }

  // Write a method that will pop a value out of our stack using the methods we've built in our singly linked list class
  pop() {
  ...
  // Write a method that will return a boolean based on whether or not the stack is empty using the methods from our singly linked list class
  youEmpty() {
    if (this.data.isEmpty()) {
      return true;
    } else {
