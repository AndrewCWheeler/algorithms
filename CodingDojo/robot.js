...
The code can comprise of a combination of our basic tool kit (setting variables, for loops, while loops, if/else if/else checks, etc.), and a handful of predefined methods, which are as follows:
...
parachuteCheck(){} -> Returns a boolean for whether or not the robot is currently on top of a parachute. Takes one unit of time.
robotCheck(){} -> Returns a boolean for whether or not the robot has found another robot. Takes one unit of time.
...
// This needs to be fixed to oscillating robots or robot catch up.
